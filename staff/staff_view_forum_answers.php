...
if (isset($_GET['post_id'])) {
    ...
    if (!$post) {
        echo "<script>alert('Forum post not found.'); window.location.href='staff_manage_forum.php';</script>";
        exit();
    }
} else {
    echo "<script>alert('Post ID not provided.'); window.location.href='staff_manage_forum.php';</script>";
    exit();
}
...
if (isset($_GET['delete_answer_id']) && isset($_GET['post_id'])) {
    ...
    if ($stmt->execute()) {
        echo "<script>alert('Answer deleted successfully!'); window.location.href='staff_view_forum_answers.php?post_id={$post_id}';</script>";
    } else {
...
<div class="wrapper">
    ...
    <div class="main-content">
        <div class="container mt-5">
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        echo "<td>
                                <a href='staff_view_forum_answers.php?post_id={$post_id}&delete_answer_id={$row['answer_number']}' class='btn btn-danger btn-sm' onclick='return confirm(\"Are you sure you want to delete this answer?\")'><i class='bi bi-trash'></i> Delete Answer</a>
                              </td>";
                        echo "</tr>";
                    }
                    ?>
            ...
            <a href="staff_manage_forum.php" class="btn btn-secondary">Back to Forum Management</a>
        </div>
    </div>
</div>
