...
if (isset($_GET['blog_id'])) {
    ...
    echo "<script>alert('Blog ID not provided.'); window.location.href='admin_manage_blog.php';</script>";
    exit();
}
...
if (isset($_GET['delete_comment_id']) && isset($_GET['blog_id'])) {
    ...
    if ($stmt->execute()) {
        echo "<script>alert('Comment deleted successfully!'); window.location.href='view_blog_comments.php?blog_id={$blog_id}';</script>";
    } else {
        echo "<script>alert('Error deleting comment.'); window.location.href='view_blog_comments.php?blog_id={$blog_id}';</script>";
    }
}
...
<div class="wrapper">
    ...
    <div class="main-content">
        <div class="container mt-5">
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        echo "<td>
                                <a href='view_blog_comments.php?blog_id={$blog_id}&delete_comment_id={$row['comment_number']}' class='btn btn-danger btn-sm' onclick='return confirm(\"Are you sure you want to delete this comment?\")'><i class='bi bi-trash'></i> Delete</a>
                              </td>";
                        echo "</tr>";
                    }
                    ?>
            ...
            <a href="admin_manage_blog.php" class="btn btn-secondary">Back to Blog Management</a>
        </div>
    </div>
</div>
